/*
Robin Healthcare is looking to hire a new programmer. However, they're having
difficulty choosing between a few of the applicants. While brainstorming intelligent ways
to decide between the applicants, they quickly decided that Seraph was right...

"You do not truly know someone until you fight them"

In that spirit, we bring you Applicant Wars, the next generation picker-chooser-thingy

-------------------------
Details:
-------------------------

Being... well... programmers, the applicants weapon of choice is the slap. Each applicant
must engage in a mono y mono match with each other applicant.

A match consists of one or more rounds where two applicants square off in a fight until one
applicant gets slapped so hard by another applicant that he or she literally can't walk. This is
serious business.

In each round, the applicants alternate slapping each other, inflicting damage, until a candidate's
health reaches or drops below 0.

After each match, the candidates are allowed to rest up to recover their health.

-------------------------
Requirements:
-------------------------

1. Design and implement a battle system based on the details above
2. Output the candidates sorted by number of wins
3. Solution must be in Javascript
*/

const candidates = [
  ["Name","Health","Damage"],
  ["Tom Cruise",136,6],
  ["Sponge Bob",110,4],
  ["James Earl Jones",175,8],
  ["Bob Barker",112,2],
  ["Tonya Harding",108,7],
  ["Charles Barkley",220,12],
  ["Peter Piper",116,4],
  ["Harry Potter",96,16],
  ["Shamu",280,24],
  ["Bill Gates",124,6],
];

var opponents = candidates.slice(2,);
var results = candidates.slice(1,);

for(var i = 1; i < candidates.length; i++){
  fight(candidates[i]);
}

results.sort(compare);

results.forEach(function(element) {
  console.log(element[0] + " : " + element[3]);
});




function compare(a, b) {
  if (a[3] > b[3])
    return -1;
  else if (a[3] < b[3])
    return 1;
  else
    return 0;
}





function fight(candidate){
  if(opponents != undefined){
    for(var i = 0; i < opponents.length; i++){
      var candidateName = candidate[0]; 
      var candidateHealth = candidate[1];
      var candidateDamage = candidate[2];
      var opponentName = opponents[i][0];
      var opponentHealth = opponents[i][1];
      var opponentDamage = opponents[i][2];      

      while(!(candidateHealth < 0 || opponentHealth < 0)){
        candidateHealth = candidateHealth - opponentDamage;
        opponentHealth = opponentHealth - candidateDamage;
      }

      (candidateHealth > opponentHealth) ? addWins(candidate) : addWins(opponents[i]);
    }
    opponents = opponents.slice(1,);
  }
}





function addWins(winner){
  if(results[results.indexOf(winner)][3] === undefined){
    results[results.indexOf(winner)].push(1);
  }else{
    results[results.indexOf(winner)][3] = results[results.indexOf(winner)][3] + 1;
  }
}